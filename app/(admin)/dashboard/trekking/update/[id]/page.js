...
export default function Trekking({ params }) {
  const { id } = params;
  const { data } = useGetTrekkingDataByIdQuery(id);
  const [updateTrekkingData, { isSuccess }] = useUpdateTrekkingDataMutation();
  ...
  useEffect(() => {
    setMap(data?.trek?.map);
    setName(data?.trek?.name);
    setTotalTime(data?.trek?.totalTime);
    setPrice(data?.trek?.price);
    setDiscount(data?.trek?.discount);
    setOverview(data?.trek?.overview);

    setComprehensiveGuide(data?.trek?.comprehensiveGuide);

    setExperienceRequired(data?.trek?.experienceRequired);
    setBestTimeToTravel(data?.trek?.bestTimeToTravel);
    setVisaAndEntryProcedure(data?.trek?.visaAndEntryProcedure);

    setHomeImageCarousel(data?.trek?.homeImageCarousel);
    setHighlights(data?.trek?.highlights);
    setTourImages(data?.trek?.tourImages);
    setItinerary(data?.trek?.itinerary);
    setPriceIncluded(data?.trek?.priceIncluded);
    setPriceExcluded(data?.trek?.priceExcluded);
    setUsefulInfo(data?.trek?.usefulInfo);
    setCountry(data?.trek?.country);
    setTrekZone(data?.trek?.trekZone);
    setFirstAidKit(data?.trek?.firstAidKit);
    setEquipment(data?.trek?.equipment);
    setOurGuides(data?.trek?.ourGuides);
  }, [data]);
  ...
  const handleUpdateData = () => {
    ...
    router.push("/dashboard/trekking");
  };
  return (
    <>
      <Container>
        ...
        <Row>
          <Col xs={12} sm={12} md={6} lg={4}>
            ...
            <Form.Control
              ...
              name="trekzone"
              onChange={(e) => {
                setTrekZone(e.target.value.toLocaleLowerCase());
              }}
            />
          </Col>
          <Col xs={12} sm={12} md={6} lg={4}>
