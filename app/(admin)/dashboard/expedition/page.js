...
const Expedition = () => {
  ...
  const { data, isLoading } = useGetExpeditionDataQuery();
  const [deleteExpeditionData, { isSuccess }] =
    useDeleteExpeditionDataMutation();
  const handleDeleteClick = (id) => {
    deleteExpeditionData(id);
  };
          ...
          {data?.allExpedition?.map((data) => {
            return (
              <tr key={data._id}>
                ...
                <td>
                  <Link href={`/dashboard/expedition/update/${data._id}`}>
                    <Button>Edit</Button>
                  </Link>
                  <Button
                    variant="danger"
