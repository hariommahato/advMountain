...
export default function Expedition({ params }) {
  const { id } = params;
  const { data } = useGetVehicleTourByIdQuery(id);
  const [updateVehicleTourData, { isSuccess, isError }] =
    useUpdateVehicleTourDataMutation();
  ...
  useEffect(() => {
    ...
    setTotalTime(data?.vehicleTour?.totalTime);
    setPrice(data?.vehicleTour?.price);
    setDiscount(data?.vehicleTour?.discount);
    setOverview(data?.vehicleTour?.overview);
    setComprehensiveGuide(data?.vehicleTour?.comprehensiveGuide);
    setExperienceRequired(data?.vehicleTour?.experienceRequired);
    setBestTimeToTravel(data?.vehicleTour?.bestTimeToTravel);
    setVisaAndEntryProcedure(data?.vehicleTour?.visaAndEntryProcedure);
    setHomeImageCarousel(data?.vehicleTour?.homeImageCarousel);
    setHighlights(data?.vehicleTour?.highlights);
    setTourImages(data?.vehicleTour?.tourImages);
    setItinerary(data?.vehicleTour?.itinerary);
    setPriceIncluded(data?.vehicleTour?.priceIncluded);
    setPriceExcluded(data?.vehicleTour?.priceExcluded);
    setUsefulInfo(data?.vehicleTour?.usefulInfo);
    setCountry(data?.vehicleTour?.country);

    setFirstAidKit(data?.vehicleTour?.firstAidKit);
    setEquipment(data?.vehicleTour?.equipment);
    setOurGuides(data?.vehicleTour?.ourGuides);
  }, [data]);
  {
    console.log(isError);
  }
