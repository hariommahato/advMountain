...
const PeakClimbing = () => {
  ...
  const { data, isLoading } = useGetPeakClimbingDataQuery();
  const [deletePeakClimbingData, { isSuccess }] =
    useDeletePeakClimbingDataMutation();
  const handleDeleteClick = (id) => {
    deletePeakClimbingData(id);
  };
          ...
          {data?.allPeakClimbing?.map((data) => {
            return (
              <tr key={data._id}>
                ...
                <td>
                  <Link href={`/dashboard/peakclimbing/update/${data._id}`}>
                    <Button>Edit</Button>
                  </Link>
                  <Button
                    variant="danger"
