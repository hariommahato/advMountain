...
export default function Hiking({ params }) {
  const { id } = params;
  const { data } = useGetHikingByIdQuery(id);
  const [updateHiking, { isSuccess, isError }] =
    useUpdateHikingMutation();
  ...
  useEffect(() => {
    setMap(data?.hiking?.map);
    setName(data?.hiking?.name);
    setTotalTime(data?.hiking?.totalTime);
    setPrice(data?.hiking?.price);
    setDiscount(data?.hiking?.discount);
    setOverview(data?.hiking?.overview);
    setComprehensiveGuide(data?.hiking?.comprehensiveGuide);
    setExperienceRequired(data?.hiking?.experienceRequired);
    setBestTimeToTravel(data?.hiking?.bestTimeToTravel);
    setVisaAndEntryProcedure(data?.hiking?.visaAndEntryProcedure);
    setHomeImageCarousel(data?.hiking?.homeImageCarousel);
    setHighlights(data?.hiking?.highlights);
    setTourImages(data?.hiking?.tourImages);
    setItinerary(data?.hiking?.itinerary);
    setPriceIncluded(data?.hiking?.priceIncluded);
    setPriceExcluded(data?.hiking?.priceExcluded);
    setUsefulInfo(data?.hiking?.usefulInfo);
    setCountry(data?.hiking?.country);

    setFirstAidKit(data?.hiking?.firstAidKit);
    setEquipment(data?.hiking?.equipment);
    setOurGuides(data?.hiking?.ourGuides);
  }, [data]);
  {
    console.log(isError);
  }
