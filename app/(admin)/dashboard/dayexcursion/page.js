...
const DayExcursion = () => {
  ...
  const { data, isLoading } = useGetDayExcursionDataQuery();
  const [deleteDayExcursionData, { isSuccess }] =
    useDeleteDayExcursionDataMutation();
  const handleDeleteClick = (id) => {
    deleteDayExcursionData(id);
  };
  ...
  return (
    <div>
      <h2>All DayExcursion Data</h2>

      <Table responsive striped="columns">
        <thead>
          <tr>
            <th>Id</th>
            <th>peakClimbing Name</th>
            <th>Country</th>
            <th>Price</th>
            <th>Discount</th>
            <th> TotalTIme</th>
          ...
          {data?.allDayExcursion?.map((data) => {
            return (
              <tr key={data._id}>
                ...
                <td>
                  <Link href={`/dashboard/dayexcursion/update/${data._id}`}>
                    <Button>Edit</Button>
                  </Link>
                  <Button
                    variant="danger"
